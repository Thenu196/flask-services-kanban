...
def db_describe():
    ...
    try:
        values = np.array(fetch_series(nom_serie), dtype=float)
...
def db_correlation():
    ...
    try:
        x = np.array(fetch_series(serie_x), dtype=float)
        y = np.array(fetch_series(serie_y), dtype=float)

        n = min(len(x), len(y))

        if n < 2:
            return jsonify({
                "erreur": "Les séries doivent contenir au moins 2 valeurs"
            }), 400
